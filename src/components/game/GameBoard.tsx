...
const GameBoard: React.FC = () => {
  ...
  const handlePlayCards = () => {
    ...
    playCards(selectedCards, claimedRank, selfPlayer.id);
    setSelectedCards([]);
  };
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="lg:flex-1 bg-game-table rounded-lg p-4 flex flex-col">
            <div className="flex-grow flex items-center justify-center">
              <div className="text-center">
                ...
                <div className="mt-4">
                  {!isSelfTurn && state.playedCards.length > 0 && (
        ...
        <div className="mt-4 bg-black/30 p-4 rounded-lg">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {selfPlayer.cards.map(card => (
              <PlayingCard
                key={card.id}
                card={card}
                selected={selectedCards.some(c => c.id === card.id)}
                onClick={() => handleCardSelect(card)}
                small={selfPlayer.cards.length > 12}
              />
            ))}
          </div>
